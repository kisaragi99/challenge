...
Completed challenge of creating React book search app with Google Books API.
## Technologies used:
- ```React```
- ```Redux```
- ```Webpack```
...
- ```docker-compose up``` - Start up the application
- ```docker-compose down --volumes``` - Stop and remove docker container
